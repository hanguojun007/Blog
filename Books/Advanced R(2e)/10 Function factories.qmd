...
因为`power1()`中的`x`只有在`square()`被调用时才会被评估。这种由于延迟评估造成的bug，在由**function factory**生成**manufactured function**时广泛存在。
...
base R 提供了一些计算最优binwidth的函数，例如`nclass.Sturges()`,`nclass.scott()`,`nclass.FD()`，我们可以将其打包成一个**function factory**。
...
gglot2 中的`ggsave()`函数，其内部使用了一个**function factory**——`plot_dev()`。 其根据文件后缀判断图片类型，选择合适的绘图设备。下面是`plot_dev()`的简化示例：
...
本节介绍一些统计分析中运用到的**factory fcuntion**例子。
...
我们将上面的函数转换为一个**function factory**，用其探索不同λ对数据的影响。
...
在统计分析工作中，我们经常需要多个Bootstrap生成器，这个时候构建一个Bootsrap生成器的**function factory**就显得很有必要。
...
最大似然估计（MLE）用来找到某个分布的参数，使得观测数据在该分布下出现概率最大。下面我们使用泊松分布来演示如何通过**factory function**优雅的实现最大似然估计。
...
虽然我们可以计算已知λ下的观察数据$X$的概率，但是最大似然估计要求我们找到一个λ，使得该概率最大。我们可以构造一个观察数据$X$的**function factory**，并使用`optimize()`函数来寻找最大概率的λ。
...
组合使用**factory function**和**functionals**，可以通过map系函数传递参数，构造出一个函数集合。如果你的**factory function**需要多个参数，就使用相应的map系函数。
