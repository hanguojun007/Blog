...
`square()`和`cube()`函数被称为**manufactured functions**，与**factory functions**相对，这种叫称呼的唯一意义就是区分函数的来源，本质都是函数，没有区别。
...
我们在之前章节中分别介绍了能够实现**factory functions**的三大基石：
...
本章，我们介绍如何使用上述的三个概念，创建**factory functions**，并将其应用到可视化与统计处理中。

在三种主要的泛函编程工具(functionals, function factories, and function operators)中，**factory functions**是使用最少的。总的来说，它们并不倾向于降低整体代码复杂度，而是将复杂度分割成更容易理解的块。函数工厂也是非常有用的函数运算符的重要组成部分，你将在第 11 章中学习。

### Outline

-   10.2节：**factory functions**如何工作和使用
-   10.3节：在可视化中的应用
-   10.4节：在统计分析中的应用
-   10.5节：与泛函联用
...
我们使用rlang包检视**factory functions**；使用ggplot2，scale包创建可视化示例。
...
实现**factory functions**的关键原理可以表述为：**factory functions**的执行环境是**manufactured functions**的创建（闭包）环境。本节将通过互动探索和一些图表帮助你更深刻地理解这一原理。
...
-   任何自由浮动的变量都存在于全局变量（**factory functions**的创建环境）中。

-   任何没有显式父级的环境都继承自全局环境（**factory functions**的创建环境）。
...
因为`power1()`中的`x`只有在`square()`被调用时才会被评估。这种由于延迟评估造成的bug，在由**factory functions**生成**manufactured functions**时广泛存在。
...
6.4.3节中讲到，函数每次执行都会创建执行环境，我们无法将函数的第一次调用与第二次调用进行关联。但是**factory functions**可以允许我们进行关联，保持每次调用时的状态。

-   **manufactured functions**的执行环境是**factory functions**的创建环境——唯一且固定。
...
本节我们给出一些应用**factory functions**到ggplot2的例子。

### Labelling

scales 包提供了许多**factory functions**，例如其中的formatter函数：它根据参数返回一个函数，用来生成不同格式的标签。
...
scales 包的这些**factory functions**，可以说就是为了ggplot2服务的，例如`laebls`参数需要提供的值就是一个函数，用来对轴标签进行格式化。
...
base R 提供了一些计算最优binwidth的函数，例如`nclass.Sturges()`,`nclass.scott()`,`nclass.FD()`，我们可以将其打包成一个**factory functions**。
...
gglot2 中的`ggsave()`函数，其内部使用了一个**factory functions**——`plot_dev()`。 其根据文件后缀判断图片类型，选择合适的绘图设备。下面是`plot_dev()`的简化示例：
...
本节介绍一些统计分析中运用到的**factory functions**例子。
...
我们将上面的函数转换为一个**factory functions**，用其探索不同λ对数据的影响。
...
在统计分析工作中，我们经常需要多个Bootstrap生成器，这个时候构建一个Bootsrap生成器的**factory functions**就显得很有必要。
...
最大似然估计（MLE）用来找到某个分布的参数，使得观测数据在该分布下出现概率最大。下面我们使用泊松分布来演示如何通过**factory functions**优雅的实现最大似然估计。
...
虽然我们可以计算已知λ下的观察数据$X$的概率，但是最大似然估计要求我们找到一个λ，使得该概率最大。我们可以构造一个观察数据$X$的**factory functions**，并使用`optimize()`函数来寻找最大概率的λ。
...
组合使用**factory functions**和**functionals**，可以通过map系函数传递参数，构造出一个函数集合。如果你的**factory functions**需要多个参数，就使用相应的map系函数。
