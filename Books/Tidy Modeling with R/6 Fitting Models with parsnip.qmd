...
合理的下一步可能是**创建参数值的可视化**。要做到这一点，将参数矩阵转换为数据框是明智的。我们可以将行名作为一列添加进去，这样它们就可以在图表中使用了。然而，请注意，现有的几个矩阵列名对于普通数据框来说不是有效的R列名（例如，`"Pr(>|t|)"`）。另一个复杂之处在于列名的一致性。对于lm对象，p值所在的列是`"Pr(>|t|)"`，但对于其他模型，可能会使用不同的检验，因此列名会有所不同（例如，`"Pr(>|z|)"`），并且检验类型会编码在列名中。
