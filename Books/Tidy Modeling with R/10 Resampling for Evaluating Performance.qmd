...
使用与线性模型相同的预测变量集（不包含额外的预处理步骤），我们可以通过"ranger"引擎（其使用“ranger”R包进行计算）将随机森林模型拟合到训练集。该模型无需预处理，因此可以使用一个简单的公式：
...
测试集的均方根误差（RMSE）估计值比训练集的0.0365差很多！这是为什么呢？
...
这两个子样本在某种程度上类似于训练集和测试集。为了避免混淆，我们使用新的术语——分析集和评估集，两者是互斥的。假设进行20次重抽样迭代。这意味着在分析集上拟合20个独立的模型，而相应的评估集则会产生20组性能统计数据。一个模型的最终性能估计值是这20个统计数据复本的平均值。这个平均值具有非常好的泛化特性，远优于重新代入估计值。不同的重采样方法会有不同的创建分析集和评估集的方法。下一节将定义几种常用的重采样方法，并讨论它们的优缺点。

### Cross-validation

交叉验证是一种成熟的重抽样方法。虽然它有多种变体，但最常见的交叉验证方法是V折交叉验证。数据被随机划分为V个大小大致相等的集合（称为折）。为了说明，在 @fig-10.2 中展示了V=3的情况，该图针对包含30个训练集样本点且折分配随机的数据。符号内的数字是样本编号。

![V-fold cross-validation randomly assigns data to folds](images/three-CV.svg){#fig-10.2}

图10.2中符号的颜色代表它们被随机分配的折数。分层抽样也是一种分配折数的方法（在5.1节中已讨论过）。

对于三重交叉验证，重采样的三次迭代如图10.3所示。在每次迭代中，一个折叠被留出用于评估统计，其余折叠则作为模型的基础。这个过程针对每个折叠持续进行，因此三个模型会产生三组性能统计数据。

![V-fold cross-validation data usage](images/three-CV-iter.svg){#fig-10.3}

当V = 3时，分析集占训练集的2/3，每个评估集都是不同的1/3。最终的重抽样性能估计值是V次重复的平均值。

使用V = 3是说明交叉验证的一个不错选择，但在实际应用中却是个糟糕的选择，因为这个值太低，无法生成可靠的估计。在实践中，V的值最常用的是5或10；我们通常倾向于将10折交叉验证作为默认选择，因为在大多数情况下，它的规模足以产生良好的结果。

改变V会有什么影响？较大的值会导致重采样估计的偏差较小，但方差较大。V的较小值则有较大的偏差，但方差较小。我们更倾向于10折交叉验证，因为重复可以减少噪声，但不能减少偏差。

主要输入是训练集数据框以及折数（默认值为10）：
...
名为splits的列包含了关于如何分割数据的信息（类似于用于创建初始训练/测试分区的对象）。虽然splits的每一行都嵌入了整个训练集的副本，但R足够智能，不会在内存中复制数据。21 tibble中的打印方法会显示每一项的频数：[2107/235]表示大约有2000个样本在分析集中，235个样本在该特定的评估集中。

这些对象还总是包含一个名为id的字符列，用于标记分区。

要手动检索分区数据，analysis()和assessment()函数会返回相应的数据框：
...
tidymodels包（例如tune）包含高级用户界面，因此像analysis()这样的函数通常不需要用于日常工作。10.3节演示了一个在这些重抽样上拟合模型的函数。

交叉验证有多种变体；我们将介绍其中最重要的几种。

#### Repeated cross-validation

交叉验证最重要的变体是重复的V折交叉验证。根据数据规模或其他特征，V折交叉验证产生的重采样估计可能会有过大的噪声。23 与许多统计问题一样，减少噪声的一种方法是收集更多数据。对于交叉验证来说，这意味着要对超过V个统计量取平均值。

为了创建
R次重复的
V折交叉验证，需要将相同的折生成过程执行
R次，以生成
R组包含
V个分区的集合。此时，不再是对
V个统计量取平均值，而是通过V
×
R
V×R个统计量来得出最终的重抽样估计值。根据中心极限定理，只要我们拥有相对于V
×
R
V×R而言足够多的数据，每个模型的汇总统计量就会趋向于正态分布。

考虑艾姆斯数据集。平均而言，10折交叉验证所使用的评估集大约包含234个属性。如果选择均方根误差（RMSE）作为统计量，我们可以将该估计值的标准差记为σ。对于简单的10折交叉验证，平均RMSE的标准误为σ/√10。如果这一结果噪声过大，重复交叉验证可以将标准误降至σ/√(10R)。对于具有R次重复的10折交叉验证，图10.4展示了标准误24随重复次数增加而快速降低的情况。

更多的重复次数对标准误差的影响往往较小。然而，如果σ的基线值大得不切实际，那么重复次数增加所带来的边际效益递减可能仍然值得额外的计算成本。
...
交叉验证的一种变体是留一法（LOO）交叉验证。如果有n个训练集样本，就会使用训练集中的n-1行数据拟合n个模型。每个模型都会对那个被排除的单一数据点进行预测。在重采样结束时，这n个预测结果会被汇总，以生成一个单一的性能统计量。

留一法（Leave-one-out methods）与几乎所有其他方法相比都存在不足。除了样本量极小的情况外，留一法（LOO）的计算量过大，而且可能不具备良好的统计特性。尽管rsample包中包含一个loo_cv()函数，但这些对象通常并未整合到更广泛的tidymodels框架中。

#### Monte Carlo cross-validation

另一种V折交叉验证的变体是蒙特卡洛交叉验证（MCCV，Xu和Liang（2001））。与V折交叉验证类似，它将固定比例的数据分配给评估集。蒙特卡洛交叉验证与常规交叉验证的不同之处在于，对于蒙特卡洛交叉验证，每次都会随机选择这一比例的数据。这导致评估集并非相互排斥。要创建这些重采样对象：
...
在第5.2节中，我们简要讨论了验证集的使用，验证集是一个单独划分出来的数据集，用于独立于测试集评估模型性能。使用验证集时，初始可用数据集会被分割为训练集、验证集和测试集（见图10.5）。

![A three-way initial split into training, testing, and validation sets](images/validation.svg){#fig-10.5}

当原始数据量非常大时，通常会使用验证集。在这种情况下，一个大型的单一分区可能足以描述模型性能，而无需进行多次重采样迭代。

借助rsample包，验证集就像其他任何重抽样对象一样；这种类型的不同之处仅在于它只有一次迭代。25图10.6展示了这种方案。
...
为了基于第5.2节的代码进行拓展，validation_set()函数可以接收initial_validation_split()的结果，并将其转换为一个rset对象，该对象与vfold_cv()等函数生成的对象类似：
...
自助重采样最初是作为一种近似统计量抽样分布的方法而发明的，这些统计量的理论性质难以处理（戴维森和欣克利，1997）。将其用于估计模型性能是该方法的次要应用。

训练集的自助抽样样本是一个与训练集大小相同但通过有放回抽样得到的样本。这意味着一些训练集数据点会被多次选入分析集。每个数据点至少被选入训练集一次的概率为63.2%。评估集包含所有未被选入分析集的训练集样本（平均而言，占训练集的36.8%）。在自助抽样中，评估集通常被称为袋外样本。

对于一个包含30个样本的训练集，图10.7展示了三个自助抽样样本的示意图。

![Bootstrapping data usage](images/bootstraps.svg){#fig-10.7}

请注意，评估集的大小各不相同。

使用rsample包，我们可以创建这样的自助法重抽样：

```{r}
bootstraps(ames_train, times = 5)
```

自助抽样产生的性能估计值方差非常小（与交叉验证不同），但存在显著的悲观偏差。这意味着，如果一个模型的真实准确率是90%，自助抽样往往会将其估计值定为低于90%。这种偏差的大小无法通过经验以足够的精度确定。此外，偏差的大小会随着性能指标的范围而变化。例如，当准确率为90%时，其偏差很可能与准确率为70%时的偏差不同。

自助法也被用于许多模型内部。例如，前面提到的随机森林模型包含1000棵独立的决策树。每棵树都是训练集不同自助样本的产物。

### Rolling forecasting origin resampling

当数据具有很强的时间成分时，重采样方法应该支持建模，以估计数据中的季节性和其他时间趋势。从训练集中随机抽样值的技术可能会破坏模型估计这些模式的能力。

滚动预测起点重采样（Hyndman 和 Athanasopoulos，2018）提供了一种方法，该方法模拟了时间序列数据在实际中通常的划分方式，即使用历史数据估计模型，并使用最新数据评估模型。对于这种类型的重采样，需要指定初始分析集和评估集的大小。重采样的第一次迭代使用这些大小，从序列的开头开始。第二次迭代使用相同的数据大小，但会按设定的样本数量进行偏移。

举例来说，一个包含15个样本的训练集进行了重采样，其中分析集大小为8个样本，评估集大小为3个样本。第二次迭代会丢弃训练集中的第一个样本，且两个数据集都向前移动一个位置。这种配置会产生5个重采样样本，如图10.8所示。

![Data usage for rolling forecasting origin resampling](images/rolling.svg){#fig-10.8}

以下是该方法的两种不同配置：

-   分析集可以累积增长（而非保持相同大小）。在第一个初始分析集之后，可以积累新样本，而不会丢弃早期数据。

-   重采样的增量不必为1。例如，对于大型数据集，增量块可以是一周或一个月，而非一天。

对于一年的数据，假设6组30天的数据块构成了分析集。对于30天的评估集且间隔29天的情况，我们可以使用rsample包来指定：
...
这一序列在每次重采样时都会重复。如果有B次重采样，那么每个性能指标就会有B个复本。最终的重采样估计值是这B个统计量的平均值。如果B = 1（如验证集的情况），那么单个统计量就代表了整体性能。

让我们重新考虑一下包含在rf_wflow对象中的先前随机森林模型。fit_resamples()函数类似于fit()，但它没有data参数，fit_resamples()有resamples，后者需要一个如本章所示的rset对象。该函数可能的接口如下：
...
还有许多其他可选参数，例如：
-   metrics：用于计算的一组性能统计指标。默认情况下，回归模型使用RMSE和R²，而分类模型计算ROC曲线下面积和总体准确率。请注意，此选择还定义了在模型评估过程中生成哪些预测。对于分类，如果只要求准确率，则不会为评估集生成类别概率估计值（因为不需要）。
...
返回值是一个与输入的重抽样数据类似的tibble（ Tibble 是 R 语言中一种数据框的变体），并附带一些额外的列：
...
由于本分析采用了10折交叉验证，因此每个训练集样本都有一个独特的预测值。这些数据可以生成有用的模型图表，以了解模型可能在哪些地方出现了问题。例如，图10.9对比了观测值和留出的预测值（类似于图9.2）：
